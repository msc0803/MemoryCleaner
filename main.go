...
// Author: mosc
// A lightweight Windows memory cleaner tool
...
func main() {
	...
	// 创建窗口
	windowTitle := syscall.StringToUTF16Ptr("Memory Cleaner - 内存清理工具 by mosc")
...
func wndProc(hwnd uintptr, msg uint32, wParam, lParam uintptr) uintptr {
	...
	case WM_PAINT:
		...
		for i, line := range lines {
			...
			rect := RECT{left: 20, top: int32(20 + i*25), right: 400, bottom: int32(20 + (i+1)*25)}
			drawText.Call(hdc, uintptr(unsafe.Pointer(&text[0])), uintptr(len(text)-1),
